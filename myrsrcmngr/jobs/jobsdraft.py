...
#turn a comma separated ip string into list of ips
def listify(string):
    str = string.split(",")
    str = [x.strip() for x in str]
    return str

#get target from resourcegroup - either subnet or ip
def get_target(self):
    if self.resourcegroup:
        target = self.resourcegroup.subnet
        if target.value_is_subnet:
            return target
        else:
            return listify(target)

#get list 
targetsOptions = [(scan.get_target(), scan.scanTemplate) for scan in scans.objects.filter(active=False).exclude(status=2) if scan.next_execution_at <= timezone.now()]
for target, option in targetsOptions:    
    print("target", target, '|', "option", option)
